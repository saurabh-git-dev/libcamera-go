...
OPENCV_DYNAMIC_MODULES="${OPENCV_DYNAMIC_MODULES:-}"

export PKG_CONFIG_PATH="${INSTALL_PREFIX}/lib/pkgconfig:${PKG_CONFIG_PATH:-}"

if [[ -z "${OPENCV_DYNAMIC_MODULES//[[:space:]]/}" ]]; then
  OPENCV_DYNAMIC_MODULES="highgui,videoio"
fi
...
if [[ -n "${OPENCV_DYNAMIC_MODULES}" ]]; then
  echo "==> Building app with hybrid OpenCV linkage"
else
  echo "==> Building app with full-static OpenCV target"
fi
...
if [[ -n "${OPENCV_MANUAL_LDFLAGS}" ]]; then
  ...
  extra_dynamic="-l:liblzma.so.5 -l:libbz2.so.1.0 -l:libzstd.so.1 -l:libLerc.so.4 -l:libjbig.so.0 -l:libdeflate.so.0 -l:libbrotlidec.so.1 -l:libbrotlicommon.so.1"

  export CGO_LDFLAGS="-Wl,-Bstatic -Wl,--start-group ${static_opencv_tokens} ${static_tokens} -Wl,--end-group -Wl,-Bdynamic ${dynamic_opencv_tokens} ${dynamic_tokens} ${extra_dynamic} ${CGO_LDFLAGS:-}"
fi
...
if [[ -n "${OPENCV_DYNAMIC_MODULES}" ]] && echo "${opencv_ldd}" | grep -Eiq 'libopencv_(highgui|videoio)'; then
  echo "Hybrid check: libopencv_highgui/videoio are dynamically linked as expected."
elif [[ -n "${OPENCV_DYNAMIC_MODULES}" ]]; then
  echo "WARNING: expected libopencv_highgui/videoio dynamic links were not found."
elif echo "${opencv_ldd}" | grep -Eiq 'libopencv_.*\.so'; then
  echo "WARNING: OpenCV shared libraries detected while full-static mode is requested."
else
  echo "Full-static check: no dynamic OpenCV libraries detected."
fi

if ! echo "${opencv_ldd}" | grep -Eiq 'libcamera\.so'; then
  echo "WARNING: libcamera dynamic link was not found."
fi
